...
// hpeError represents an error responses from HPE Storage API.
type hpeError struct {
	// List of errors returned by the HPE Storage API.
	Errors []struct {
...
// Error returns the first error message from the HPE Storage API error.
...
// isHpeErrorOf checks if the given error is of type hpeError, has the specified status code,
...
// getTarget retrieves the qualified name and addresses of HPE Storage target for the configured mode.
func (p *hpeClient) getTarget() (targetQN string, targetAddrs []string, err error) {
	connector, err := p.driver.connector()
	if err != nil {
		return "", nil, err
	}
	...
	targetAddrs = shared.SplitNTrimSpace(p.driver.config["hpe.target"], ",", -1, true)
	...
	var nq string
	for _, iface := range interfaces {
		var resp hpeResponse[hpePort]

		url := api.NewURL().Path("ports").WithQuery("filter", "name='"+iface.Name+"'")
		err = p.requestAuthenticated(http.MethodGet, url.URL, nil, &resp)
		if err != nil {
			return "", nil, fmt.Errorf("Failed to retrieve HPE Storage targets: %w", err)
		}
